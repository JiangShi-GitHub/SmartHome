...
void bmp_display(const char* bmp_file, int x0, int y0)
{
    ...
    for(int y = 0; y < abs(height); y++)
    {
        for(int x = 0; x < abs(width); x++)
        {
            ...
            lcd_draw_point(x0 + x, y0 + height - 1 - y, color);
        }
        i += laizi;
    }
...
/*
    ...
    @target：目标页面
*/
void bmp_change_pages(Pages now, Pages target)
{
    if(now.sign < target.sign)
    {
        for(int i = 0, j = 800; j >= 0; i -= 25, j -= 25)
        {
            bmp_display(now.__FILE, i, 0);
            bmp_display(target.__FILE, j, 0);
        }
    }
    else if(now.sign > target.sign)
    {
        for(int i = -800, j = 0; j <= 800; i += 25, j += 25)
        {
            bmp_display(now.__FILE, j, 0);
            bmp_display(target.__FILE, i, 0);
        }
    }
}